    ...
    def step(self):
        for i, body_path in enumerate(self.body_paths):
            for j in body_path.iterator():
                ...
                wait_if_gui(msg)
                #print(msg)
    def execute(self, time_step=0.05, joint_position_publisher=None):
        for i, body_path in enumerate(self.body_paths):
            for j in body_path.iterator():
                #time.sleep(time_step)
                if joint_position_publisher is not None:
                    joint_position_publisher.publish(Float64MultiArray(data=body_path.path[j][:7]))
...
def get_ik_fn(robot, fixed=[], teleport=False, num_attempts=10):
    ...
    def fn(body, pose, grasp):
        ...
        for _ in range(num_attempts):
            ...
            if (q_approach is None) or pairwise_collisions(robot, obstacles):
                continue
            conf = BodyConf(robot, q_approach)
            q_grasp = inverse_kinematics(robot, grasp.link, gripper_pose)
            if (q_grasp is None): # or pairwise_collisions(robot, obstacles):
                continue
            if teleport:
            ...
            command = Command([BodyPath(robot, path),
                            ...
                            #    BodyPath(robot, path[::-1], attachments=[grasp])])
                               BodyPath(robot, path[::-1], attachments=[])])
            return (conf, command)
            # TODO: holding collisions
        return None
    return fn

def get_grasping_fn(robot, fixed=[], teleport=False, num_attempts=10):
    movable_joints = get_movable_joints(robot)
    sample_fn = get_sample_fn(robot, movable_joints)
    def fn(body, pose, grasp):
        # obstacles = [body] + fixed
        ...
        for _ in range(num_attempts):
            ...
            #path = workspace_trajectory(robot, grasp.link, point_from_pose(approach_pose), -direction,
            #                                   quat_from_pose(approach_pose))
            # path = plan_direct_joint_motion(robot, conf.joints, q_approach, obstacles=obstacles)
            path = plan_direct_joint_motion(robot, conf.joints, q_approach, obstacles=[])
            if path is None:
                if DEBUG_FAILURE: wait_if_gui('Approach motion failed')
                continue
            # command = Command([BodyPath(robot, path),
            #                    Attach(body, robot, grasp.link),
            #                    BodyPath(robot, path[::-1], attachments=[grasp])])
            approach = Command([BodyPath(robot, path)])
            leaving = Command([BodyPath(robot, path[::-1])])
            return (conf, BodyConf(robot, q_approach), leaving)
            # TODO: holding collisions
        return None
    return fn
...
def get_free_motion_gen(robot, fixed=[], teleport=False, self_collisions=True, smoothing=False, algorithm=None):
...
def get_holding_motion_gen(robot, fixed=[], teleport=False, self_collisions=True, smoothing=False, algorithm=None):
    def fn(conf1, conf2, body, grasp, fluents=[]):
        ...
        if teleport:
            ...
            path = plan_joint_motion(robot, conf2.joints, conf2.configuration,
                                     obstacles=obstacles, attachments=[grasp.attachment()], self_collisions=self_collisions, algorithm=algorithm)
            if path is None:
