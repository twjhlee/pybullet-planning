...
camera = p0.loadURDF("models/franka_description/robots/camera.urdf")

# kuka = p0.loadURDF("kuka_iiwa/model.urdf")
panda = p1.loadURDF("models/franka_description/robots/panda_arm_hand.urdf")

ed0 = ed.UrdfEditor()
ed0.initializeFromBulletBody(panda, p1._client)
ed1 = ed.UrdfEditor()
ed1.initializeFromBulletBody(camera, p0._client)
#ed1.saveUrdf("combined.urdf")

parentLinkIndex = 8

jointPivotXYZInParent = [0.0, 0.0, 0.0]
jointPivotRPYInParent = [0, 0, 0]

jointPivotXYZInChild = [0.0, 0.05, 0.0]
jointPivotRPYInChild = [0, 0, -np.pi / 4]
